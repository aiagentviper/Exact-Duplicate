  ...
  return (
    <div className={`relative w-full overflow-hidden rounded-2xl bg-zinc-900/50 ${ratio}`}>
      <img
        src={img}
        alt={title}
        className="h-full w-full object-cover object-center grayscale contrast-[1.2] brightness-[0.82]"
      />
      <div className="absolute inset-0 bg-black/20" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-transparent" />
      <div className="absolute inset-x-0 bottom-4 flex justify-center px-3">
        <span className="inline-flex w-[90%] items-center justify-center gap-1.5 rounded-full border border-white/20 bg-white/10 px-4 py-2 font-inter text-[11px] text-white backdrop-blur-md">
          View Case Study ↗
        </span>
      </div>
    </div>
