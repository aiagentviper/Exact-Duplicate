  ...
  return (
    <section className="border-t border-border/40 w-full">
      <div className="grid lg:grid-cols-[1.05fr_0.95fr] min-h-[520px] w-full">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative overflow-hidden min-h-[320px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 border-t border-border/40 w-full">
        {serviceCards.map((card, i) => (
          <motion.div
            ...
            className={`p-5 lg:p-6 border-border/40 hover:bg-card/50 transition-colors ${i % 2 === 0 ? 'md:border-r' : ''} ${i < 2 ? 'border-b' : ''}`}
          >
            <div className="rounded-2xl border border-border/40 bg-card/30 p-5 h-full">
